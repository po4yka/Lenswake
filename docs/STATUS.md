...
An apparent blocker recorded on 2026-09-06 — `Accessibility runtime connection` permanently Blocked
on the Pixel 8 Pro — was a test-procedure defect, not an application defect. `onServiceConnected()`
runs, `attach()` sets the state, and the preflight observes it on the same singleton in the same
process. The earlier readings were taken after an `am force-stop`, which kills the process and leaves
Android without rebinding the accessibility service, so the app was reporting the truth.

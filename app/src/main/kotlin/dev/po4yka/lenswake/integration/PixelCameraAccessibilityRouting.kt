...
internal val openNightSightControlAction = AutomationAction.OPEN_NIGHT_SIGHT_TIME_LAPSE_CONTROL
internal val selectSpeedAction = AutomationAction.SELECT_TIME_LAPSE_SPEED

internal val speedSignals =
    TimeLapseSpeed.entries.associateBy {
        it.pixelCameraContract.activeSignal
    }

internal val lensSignals =
    LensSelection.entries.associateBy {
        it.pixelCameraContract.activeSignal
    }

internal val lensActions =
    LensSelection.entries.associateWith {
...
internal val stopOptionalSignals =
    speedSignals.keys +
        setOf(
...
internal fun missingActionFailure(action: AutomationAction): AutomationFailure =
    AutomationFailure(
        code =
            when (action) {

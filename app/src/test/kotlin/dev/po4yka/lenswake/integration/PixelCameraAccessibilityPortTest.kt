...
class PixelCameraAccessibilityPortTest : PixelCameraAccessibilityPortTestFixture() {
    @Test
    fun `configured profile gesture is dispatched when no semantic target is available`() =
        runTest {
    ...
    @Test
    fun `configured speed profile gesture is dispatched when speed selector is absent`() =
        runTest {
            ...
            val result =
                port(gateway = gateway).selectTimeLapseSpeed(
    ...
    @Test
    fun `configured profile gesture follows a rejected node-bounds gesture`() =
        runTest {
            val fallbackPoint = NormalizedPoint(x = 0.44f, y = 0.66f)
            val gateway =
                FakeAccessibilityGateway(
    ...
    @Test
    fun `ambiguous semantic target does not fall through to profile gesture`() =
        runTest {
            val fallbackPoint = NormalizedPoint(x = 0.44f, y = 0.66f)
    ...
    @Test
    fun `below-threshold semantic candidate does not fall through to profile gesture`() =
        runTest {
            val fallbackPoint = NormalizedPoint(x = 0.44f, y = 0.66f)
                        ...
                        profile().targets +
                            mapOf(
                                AutomationAction.SELECT_REAR_MAIN_LENS to
                                    selectorSet(LENS_ACTION_RESOURCE).copy(
                                        minimumScore = 110,
                                    ),
                            ),
                    fallbackGestures =
...
class PixelCameraAccessibilityDialogTest : PixelCameraAccessibilityPortTestFixture() {
    private fun dialogFixtureProfile(
        dialog: PixelCameraDialogKind,
        messageResource: String,
        message: String,
        recoveryResource: String,
    ): PixelCameraProfile =
        profile().copy(
            dialogProfiles =
                mapOf(
                    dialog to
                        PixelCameraDialogProfile(
                            presence =
                                UiSelectorSet(
                                    selectors =
                                        listOf(
                    ...
                    PixelCameraDialogKind.UNKNOWN to
                        PixelCameraDialogProfile(
                            presence =
                                UiSelectorSet(
                                    selectors =
    ...
    @Test
    fun `typed dialog takes precedence over the generic unknown rule`() =
        runTest {
            ...
            val configuredProfile = dialogFixtureProfile(dialog, messageResource, message, recoveryResource)
            val messageNode =
                node(messageResource).copy(
                    role = "android.widget.TextView",
                    text = message,
                    clickable = false,
                )
    ...
    @Test
    fun `typed dialog is observed and recovered through its dedicated target`() =
        runTest {
            val dialog = PixelCameraDialogKind.VIDEO_DURATION_LIMIT_REACHED
    ...
    @Test
    fun `unknown dialog fails closed without dispatching a recovery target`() =
        runTest {
...
class PixelCameraAccessibilityStateTest : PixelCameraAccessibilityPortTestFixture() {
    @Test
    fun `time lapse reports rear main lens only when its profile signal matches`() =
        runTest {
            val gateway =
                FakeAccessibilityGateway(
                    activeSignals(
    ...
    @Test
    fun `video-only profile is inspected without unrelated Time Lapse signals`() =
        runTest {
            val videoProfile =
                profile().copy(
                    targets =
    ...
    @Test
    fun `video inspection and recording controls use video-specific profile routes`() =
        runTest {
            val videoProfile =
                profile().copy(
                    targets =
    ...
    private fun nightRouteProfile(): PixelCameraProfile =
        profile().copy(
            targets =
                mapOf(
    ...
    @Test
    fun `night sight inspection and recording controls use night-specific profile routes`() =
        runTest {
            val nightProfile = nightRouteProfile()
            val gateway =
                FakeAccessibilityGateway(
                    nodes =
                        activeSignals(
    ...
    private fun openNightSightControlProfile(): PixelCameraProfile =
        profile().copy(
            targets =
                mapOf(
                    AutomationAction.OPEN_NIGHT_SIGHT_TIME_LAPSE_CONTROL to
                        selectorSet(OPEN_NIGHT_ACTION_RESOURCE),
                    AutomationAction.SELECT_REAR_ULTRAWIDE_LENS to selectorSet(ULTRAWIDE_ACTION_RESOURCE),
                    AutomationAction.START_NIGHT_SIGHT_TIME_LAPSE_RECORDING to
                        selectorSet(START_NIGHT_ACTION_RESOURCE),
                ),
    ...
    private fun openNightSightControlGateway(): FakeAccessibilityGateway =
        FakeAccessibilityGateway(
            nodes =
                activeSignals(
                    PixelCameraStateSignal.TIME_LAPSE_MODE_ACTIVE,
                    ...
                    PixelCameraStateSignal.REAR_ULTRAWIDE_LENS_ACTIVE,
                    PixelCameraStateSignal.NOT_RECORDING,
                ) + listOf(node(OPEN_NIGHT_ACTION_RESOURCE)),
            dispatchResult = AccessibilityDispatchResult.SemanticActionDispatched,
        )

    @Test
    fun `open night sight control row infers the control state`() =
        runTest {
            val port = port(gateway = openNightSightControlGateway())

            val observed =
                assertInstanceOf(
                    PortResult.Observed::class.java,
                    port.inspect(profileUse(openNightSightControlProfile())),
                )
            assertEquals(
    ...
    @Test
    fun `the night sight control opener routes through its profile action`() =
        runTest {
            val profile = openNightSightControlProfile()
            val gateway = openNightSightControlGateway()

            assertInstanceOf(
                ActionDispatch.Dispatched::class.java,
                port(gateway = gateway).openNightSightTimeLapseControl(profileUse(profile)),
            )
            assertEquals(
                listOf("node-$OPEN_NIGHT_ACTION_RESOURCE"),
                gateway.clickedNodes.map(UiNodeSnapshot::id),
    ...
    @Test
    fun `night sight mode signal wins over the shared time lapse mode chip`() =
        runTest {
            val nightProfile =
                profile().copy(
                    targets =
...
class PixelCameraAccessibilityFailureTest : PixelCameraAccessibilityPortTestFixture() {
    @Test
    fun `inspection rejects an accessibility snapshot whose root could not refresh`() =
        runTest {
            val result =
                port(
                    gateway =
    ...
    @Test
    fun `dispatch rejects when the active root could not refresh before path resolution`() =
        runTest {
            val gateway =
                FakeAccessibilityGateway(
                    nodes = listOf(node(LENS_ACTION_RESOURCE)),
    ...
    @Test
    fun `dispatch reports a typed failure when the live node no longer matches the selected target`() =
        runTest {
            val gateway =
                FakeAccessibilityGateway(
                    nodes = listOf(node(LENS_ACTION_RESOURCE)),
    ...
    @Test
    fun `semantic action remains preferred over configured profile gesture`() =
        runTest {
            val gateway =
                FakeAccessibilityGateway(
                    nodes = listOf(node(LENS_ACTION_RESOURCE)),
    ...
    @Test
    fun `profile gesture fails when Pixel Camera is no longer foreground`() =
        runTest {
            val gateway =
                FakeAccessibilityGateway(
                    nodes = emptyList(),
    ...
    @Test
    fun `close speed control dispatches global Back without selecting a node`() =
        runTest {
            val gateway =
                FakeAccessibilityGateway(
                    nodes =
    ...
    @Test
    fun `close speed control maps rejected global Back to typed failure`() =
        runTest {
            val gateway =
                FakeAccessibilityGateway(
                    nodes =
    ...
    @Test
    fun `close speed control refuses global Back when fresh picker state changed`() =
        runTest {
            val gateway =
                FakeAccessibilityGateway(
                    nodes =
...
class PixelCameraAccessibilityStateSafetyTest : PixelCameraAccessibilityPortTestFixture() {
    @Test
    fun `time lapse lens remains unknown when configured rear main signal does not match`() =
        runTest {
            val gateway =
                FakeAccessibilityGateway(
                    activeSignals(
    ...
    @Test
    fun `recording with obscured speed and lens controls is exposed as safe stop state`() =
        runTest {
            val optionalControls =
                listOf(
                    PixelCameraStateSignal.TIME_LAPSE_SPEED_X120_ACTIVE,
                    PixelCameraStateSignal.REAR_MAIN_LENS_ACTIVE,
                )
    ...
    @Test
    fun `recording keeps exact time lapse state when speed and lens are observable`() =
        runTest {
            val gateway =
                FakeAccessibilityGateway(
                    activeSignals(
    ...
    @Test
    fun `recording tolerates ambiguous exposed speed or lens controls only for safe stop`() =
        runTest {
            val optionalControls =
                listOf(
                    PixelCameraStateSignal.TIME_LAPSE_SPEED_X120_ACTIVE,
    ...
    @Test
    fun `non-recording state still rejects ambiguous speed controls`() =
        runTest {
    ...
    @Test
    fun `recording state still rejects an ambiguous stop control`() =
        runTest {
    ...
    @Test
    fun `partially configured future lens does not break supported capture inspection`() =
        runTest {
            val baseline = profile()
            val partiallyConfiguredFrontVideo =
                baseline.copy(
                    targets =
                        baseline.targets +
                            mapOf(
                                AutomationAction.SELECT_FRONT_LENS to
                                    baseline.targets.getValue(AutomationAction.SELECT_VIDEO),
                                AutomationAction.START_VIDEO_RECORDING to
                                    baseline.targets.getValue(AutomationAction.START_RECORDING),
                                AutomationAction.STOP_VIDEO_RECORDING to
                                    baseline.targets.getValue(AutomationAction.STOP_RECORDING),
                            ),
...
class PixelCameraAccessibilityActionRoutingTest : PixelCameraAccessibilityPortTestFixture() {
    @Test
    fun `4k and 60 fps actions use fresh profile-defined targets`() =
        runTest {
            ...
            assertInstanceOf(
                ActionDispatch.Dispatched::class.java,
                port.selectVideoResolution4k(profileUse(configured)),
            )
    ...
    @Test
    fun `rear main lens action dispatches only its profile-defined target`() =
        runTest {
            val target =
                node(LENS_ACTION_RESOURCE).copy(
    ...
    @Test
    fun `speed control opener and speed option are separate semantic dispatches`() =
        runTest {
            val gateway =
                FakeAccessibilityGateway(
    ...
    @Test
    fun `visible speed option is observable picker-open state without claiming it selected`() =
        runTest {
            val gateway =
                FakeAccessibilityGateway(
                    activeSignals(
    ...
    @Test
    fun `persistent speed picker reports the selected time lapse speed`() =
        runTest {
            val gateway =
                FakeAccessibilityGateway(
                    activeSignals(
    ...
    @Test
    fun `inspection ignores incomplete unsupported time lapse capability`() =
        runTest {
            val withoutPickerSignal =
                profile().copy(
...
class PixelCameraAccessibilityProfileValidationTest : PixelCameraAccessibilityPortTestFixture() {
    ...
    fun `beta calibration profile is rejected before action dispatch`() =
        runTest {
            ...
            val result =
                productionPort.selectTimeLapse(
    ...
    @Test
    fun `unapproved system build is incompatible before accessibility inspection`() =
        runTest {
            val gateway = FakeAccessibilityGateway(activeSignals())
            val current =
                environment().copy(
    ...
    @Test
    fun `stored probable compatibility requires rehearsal even on exact environment`() =
        runTest {
            val gateway = FakeAccessibilityGateway(activeSignals())
    ...
    @Test
    fun `unsupported selector schema is rejected before accessibility inspection`() =
        runTest {
            val gateway = FakeAccessibilityGateway(activeSignals())
            val unsupported =
                profile().copy(
    ...
    @Test
    fun `rehearsal admits every non-incompatible persisted compatibility`() =
        runTest {
    ...
    @Test
    fun `rehearsal rejects incompatible environment before accessibility inspection`() =
        runTest {
            val gateway = FakeAccessibilityGateway(activeSignals())
            val otherDevice = environment().copy(deviceModel = "Pixel 9 Pro")

            val result =
                port(currentEnvironment = otherDevice, gateway = gateway).inspect(
    ...
    @Test
    fun `rehearsal rejects a profile explicitly marked incompatible`() =
        runTest {
            val gateway = FakeAccessibilityGateway(activeSignals())
    ...
    @Test
    fun `rehearsal rejects wrong camera package before environment inspection`() =
        runTest {
...
abstract class PixelCameraAccessibilityPortTestFixture {
    ...
    protected fun environment(): PixelCameraEnvironment =
    ...
    protected fun node(resourceId: String): UiNodeSnapshot =
        UiNodeSnapshot(
            id = "node-$resourceId",
            packageName = CAMERA_PACKAGE,
            resourceId = resourceId,
    ...
    protected companion object {
        ...
        const val OPEN_NIGHT_ACTION_RESOURCE = "profile.night.open"

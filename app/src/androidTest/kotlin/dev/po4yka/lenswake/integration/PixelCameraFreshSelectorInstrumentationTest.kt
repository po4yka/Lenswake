...
class PixelCameraFreshSelectorInstrumentationTest {
    @Test
    fun everyCaptureConfigurationActionResolvesASecondFreshNodeBeforeDispatch() =
        runBlocking {
    ...
    private fun routingCases(): List<RoutingCase> =
        buildList {
            add(
                actionCase("video-mode", AutomationAction.SELECT_VIDEO) { port, use ->
                    port.selectVideo(use)
                },
            )
            add(
                actionCase("time-lapse-mode", AutomationAction.SELECT_TIME_LAPSE) { port, use ->
                    port.selectTimeLapse(use)
                },
            )
            add(
                actionCase("night-sight-time-lapse", AutomationAction.SELECT_NIGHT_SIGHT_TIME_LAPSE) { port, use ->
            ...
            add(
                ...
                ) { port, use -> port.openNightSightTimeLapseControl(use) },
            )
            add(
                actionCase(
                    "night-sight-start",
            ...
            add(
                actionCase("time-lapse-speed-control", AutomationAction.OPEN_TIME_LAPSE_SPEED_CONTROL) { port, use ->
                    port.openTimeLapseSpeedControl(use)
                },
            )
            add(
                actionCase("video-resolution-4k", AutomationAction.SELECT_VIDEO_RESOLUTION_4K) { port, use ->
                    port.selectVideoResolution4k(use)
                },
            )
            add(
                actionCase("video-frame-rate-60", AutomationAction.SELECT_VIDEO_FRAME_RATE_60) { port, use ->
    ...
    private fun selector(resourceId: String): UiSelectorSet =
        UiSelectorSet(
            selectors =
                listOf(
                    UiSelector(
    ...
    private fun actionResource(action: AutomationAction): String = "instrumentation.action.${action.name.lowercase()}"

    private fun speedResource(speed: TimeLapseSpeed): String = "instrumentation.speed.${speed.name.lowercase()}"

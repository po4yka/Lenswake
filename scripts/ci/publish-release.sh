...
tag="${1:?Usage: publish-release.sh TAG APK CHECKSUMS}"
apk="${2:?Usage: publish-release.sh TAG APK CHECKSUMS}"
checksums="${3:?Usage: publish-release.sh TAG APK CHECKSUMS}"
...
gh release upload "$tag" "$apk" "$checksums" --repo "$repository" --clobber

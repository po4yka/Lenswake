...
private suspend fun EngineEnvironment.convergeStartStep(
    ...
    when (observed) {
        PixelCameraState.Photo -> {
            selectInitialMode(context, capture)
        }

        is PixelCameraState.Video -> {
            convergeVideo(context, capture, observed)
        }

        is PixelCameraState.TimeLapse -> {
            convergeTimeLapse(context, capture, observed)
        }

        is PixelCameraState.NightSightTimeLapse -> {
            convergeNightSight(context, capture, observed)
        }

        is PixelCameraState.NightSightTimeLapseControl -> {
            convergeNightSightControl(context, capture, observed)
        }

        is PixelCameraState.TimeLapseSpeedPicker -> {
            convergeSpeedPicker(context, capture, observed)
        }
...
private suspend fun EngineEnvironment.selectInitialMode(
    ...
    // Night Sight Time Lapse shares the Time Lapse mode chip and only exposes its control row
    // inside that mode, so both capture kinds converge through Video into Time Lapse first.
    val initialMode =
        if (capture is CaptureConfiguration.TimeLapse || capture is CaptureConfiguration.NightSightTimeLapse) {
            CaptureMode.VIDEO
        } else {
            capture.mode
        }
...
private suspend fun EngineEnvironment.convergeTimeLapse(
    ...
    return when {
        ...
        observed.matchesRecording(capture) -> {
            fail(
                context,
                failure(
        ...
        observed.speed != capture.speed -> {
            openSpeedPicker(context)
        }

        else -> {
            startAndVerifyRecording(context, capture)
        }
    }
}

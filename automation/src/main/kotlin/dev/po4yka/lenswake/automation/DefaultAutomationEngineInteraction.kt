...
internal suspend fun EngineEnvironment.launchAndObserveCamera(
    ...
    return observeCamera(
        ...
        state =
            if (launchState == AutomationStateName.LOCATING_PIXEL_CAMERA) {
...
private suspend fun EngineEnvironment.inspectSpeedPicker(
    ...
    return when (inspection) {
        is PortResult.Observed -> {
            PickerInspection(predicate(inspection.value)).also { result ->
                if (result.open) {
                    context.transition(
                        state,
                        operation = operation,
        ...
        is PortResult.Unavailable -> {
            PickerInspection(open = false, failure = inspection.failure)
        }
    }
}
...
/**
 * The Night Sight control opener mirrors the speed-picker opener: a dispatched tap may be ignored
 * while Pixel Camera settles, so convergence retries the opener instead of only polling, and the
 * opener is idempotent because it inspects the control row before every dispatch.
 */
internal suspend fun EngineEnvironment.openNightSightTimeLapseControlAndVerify(
    context: RunContext,
    dispatchFailure: AutomationFailure,
    verificationFailure: AutomationFailure,
    predicate: (PixelCameraState) -> Boolean,
) {
    val operation = AutomationOperation.OPEN_NIGHT_SIGHT_TIME_LAPSE_CONTROL
    val actionState = AutomationStateName.OPENING_NIGHT_SIGHT_TIME_LAPSE_CONTROL
    val policy = config.policyFor(operation)
    var lastFailure: AutomationFailure? = null
    for (attempt in 1..policy.maxAttempts) {
        if (attempt > 1) {
        ...
        val beforeDispatch = inspectNightSightControl(context, attempt, verificationFailure, predicate)
        if (beforeDispatch.open) return
        lastFailure = beforeDispatch.failure ?: lastFailure

        val dispatch = dispatchNightSightControlOpen(context, attempt, dispatchFailure)
        if (dispatch is ActionDispatch.Rejected) {
            lastFailure = dispatch.failure
            continue
        }
        val afterDispatch = inspectNightSightControl(context, attempt, verificationFailure, predicate)
        if (afterDispatch.open) return
        lastFailure = afterDispatch.failure ?: lastFailure
    }
    fail(context, lastFailure ?: verificationFailure)
...
private data class NightSightControlInspection(
    val open: Boolean,
    val failure: AutomationFailure? = null,
)

private suspend fun EngineEnvironment.inspectNightSightControl(
    context: RunContext,
    attempt: Int,
    verificationFailure: AutomationFailure,
    predicate: (PixelCameraState) -> Boolean,
): NightSightControlInspection {
    val operation = AutomationOperation.OPEN_NIGHT_SIGHT_TIME_LAPSE_CONTROL
    val state = AutomationStateName.VERIFYING_NIGHT_SIGHT_TIME_LAPSE_CONTROL
    context.transition(state, operation = operation, outcome = AutomationOutcome.STARTED, attempt = attempt)
    val inspection =
        inspectCameraHandlingDialog(
            context = context,
    ...
    return when (inspection) {
        is PortResult.Observed -> {
            NightSightControlInspection(predicate(inspection.value)).also { result ->
                if (result.open) {
                    context.transition(
                        state,
                        operation = operation,
        ...
        is PortResult.Unavailable -> {
            NightSightControlInspection(open = false, failure = inspection.failure)
        }
    }
}

private suspend fun EngineEnvironment.dispatchNightSightControlOpen(
    context: RunContext,
    attempt: Int,
    dispatchFailure: AutomationFailure,
): ActionDispatch {
    val operation = AutomationOperation.OPEN_NIGHT_SIGHT_TIME_LAPSE_CONTROL
    val state = AutomationStateName.OPENING_NIGHT_SIGHT_TIME_LAPSE_CONTROL
    context.transition(state, operation = operation, outcome = AutomationOutcome.STARTED, attempt = attempt)
    var durationMs: Long? = null
    val dispatch =
        safeCall(
            block = {
                when (
                    val timed =
                        timed(operation) {
                            pixelCamera.openNightSightTimeLapseControl(context.profileUse)
                        }
                ) {
                    is TimedCall.Completed -> timed.value.also { durationMs = timed.durationMs }
                    TimedCall.TimedOut -> ActionDispatch.Rejected(timeoutFailure(operation))
